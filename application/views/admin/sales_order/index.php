<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-success">Semua Sales Order</h6>
        </div>
        <div class="card-body table-responsive">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    <?php if (empty($orders)): ?>
                        <tr><td colspan="7" class="text-center text-gray-500">Belum ada order</td></tr>
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($o->status == 'draft'): ?>
                                    <a href="<?= site_url('admin/sales_order_ubah_status/' . $o->id_order . '/dikirim') ?>"
                                       class="btn btn-primary btn-sm"
                                       onclick="return confirm('Ubah status ke Dikirim?')">
                                        <i class="fas fa-paper-plane"></i> Kirim
                                    </a>
                                    <a href="<?= site_url('admin/sales_order_ubah_status/' . $o->id_order . '/dibatalkan') ?>"
                                       class="btn btn-danger btn-sm"
                                       onclick="return confirm('Batalkan order ini?')">
                                        <i class="fas fa-times"></i> Batal
                                    </a>
                                <?php elseif ($o->status == 'dikirim'): ?>
                                    <a href="<?= site_url('admin/sales_order_ubah_status/' . $o->id_order . '/selesai') ?>"
